    ...
    override fun onChildAdded(snapshot: DataSnapshot, previousChildKey: String?) {
        ...
        if (filter == null) {
            ...
            if (child.size>1){
                ...
                if (filtered1.lowercase(Locale.getDefault()).contains(filter.lowercase(Locale.getDefault())) ||
                        filtered2.lowercase(Locale.getDefault()).contains(filter.lowercase(Locale.getDefault()))) {
                    mSnapshots.add(index, snapshot)
                    notifyOnChildChanged(ChangeEventType.ADDED, snapshot, index, -1)
                }
            }else{
